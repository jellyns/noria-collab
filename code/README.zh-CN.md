...
卡片标题栏统一提供图标、添加和 **…** 菜单；同排卡片等高，底部可调高度。主页和日记可就地添加、配置、复制与恢复卡片，日 / 周 / 月 / 年分别沿用各自布局。笔记、Base 与 Dataview 查询可作为卡片显示，依赖安装复用 Obsidian 的插件管理。

Markdown 小组件可以展示由其他流程生成的每日简报、当前建议、每周回顾或项目监控笔记。Noria 只读取并链接配置好的 `.md` 文件，不要求接入模型，也不会自行生成这些内容。

项目、日记、看板与时间轴共用任务编辑器：勾选即可完成，点击标题修改任务、日期、状态和标签，重复与依赖按需展开。**记下一步**写入项目既有任务笔记，编辑器保留源笔记入口。

MOC 入口按网格对齐，用小色点标识主题，同名 Canvas 保留独立图标。**+** 可选择已有笔记，**…** 集中管理顺序和颜色。项目、MOC、工作流及 Base 来源使用独立标签页，已打开的来源会直接复用，主页与项目输入保持。

![主页 MOC 入口](docs/assets/zh-CN/home-moc.png)
...
侧边栏日历用于定位周期笔记，可以使用 Noria 托管路径、Obsidian Daily Notes，或自定义目录与命名格式。本地体验包中的日、周、月、年入口进入日记工作区，缺失笔记在首次保存时创建；季度和关闭主页模块后的原生入口保留原有创建设置。

### 习惯

习惯系统把“今天是否完成”和“长期是否形成”分开呈现。主页习惯卡可直接打卡，默认保留最近 21 天；**… → 卡片设置 → 显示天数**可选 7、14、21、30、60、90 天。窄卡横向滚动历史、固定名称并先显示今天。记录保存在日期 Markdown 中，热力图和汇总趋势按需查看。

![主页习惯卡](docs/assets/zh-CN/home-habits.png)

### 趋势统计

趋势统计用于观察一段时间内的变化，而不是每天评价自己。笔记增长、任务完成、习惯与工作量热力图、知识库分布和日态信号共享同一时间范围，也可以作为独立主页卡片重新排列。

可自定义日记记录的名称、类型、选项与单位，再从同一属性添加趋势或分布卡。统计也支持任意笔记属性和文件夹；点结果即可追溯原记录。倒计时支持开始 / 截止日期、拖动改期和原文档归档。上述卡片与记录改进随当前体验构建提供。

![配置统计卡](docs/assets/zh-CN/card-statistics.png)

### 日记与复盘

工作台顶部的**主页 / 日记**导航保持可见。日记内通过**记录 / 复盘**切换阅读与书写内容，共用当前周期、日期和 Markdown 来源。主页其余卡片保持。
...
三件事、待办和习惯沿用主页卡片样式。卡片右上角 **…** 可调整宽度、顺序或折叠；日、周、月、年分别共用布局，切日期只切换内容。

![日记工作区](docs/assets/zh-CN/diary-workspace.png)

随手记把念头、链接和换行原样留在所选日的 Inbox；主页和 **Noria：随手记**命令记录到今天。心情与精力可在日记旁填写，日态图上的日期能直接返回当天记录。

![随手记与日态](docs/assets/zh-CN/diary-quick-notes.png)

*示例内容；此工作台更新随本地体验包提供。* [日记使用说明](docs/zh-CN/USER-GUIDE.md#5-日历与日记)
...
*此处为复盘改版的示例内容，当前随本地体验包提供。* [复盘使用说明](docs/zh-CN/USER-GUIDE.md#8-复盘中心)
