...
test("inbox starter notes and base seeds write configured status ids instead of Chinese legacy values", () => {
  ...
  assert.equal(plugin.getSupportSeedDefinitions().some(row=>row.key==="starterInboxNote"),false);
...
test("home inbox rows expose gentle rhythm cues without a separate suggestion panel", async () => {
  ...
  assert.deepEqual(rhythmChips.map((node) => node?.textContent), [undefined, "推进", "稍后"]);
  assert.deepEqual(rhythmChips.map((node) => node?.attrs["data-noria-inbox-rhythm"]), [undefined, "next", "later"]);
